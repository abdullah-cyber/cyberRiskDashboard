export default function ThreatTable({ threats, isLoading, error, onRowClick }) {
  return (
      ...
      <table className="w-full text-sm text-left border-collapse table-fixed">
        ...
        <tbody>
          {isLoading ? (
            ...
            <tr><td colSpan={5} className="py-6 text-center text-red-500">Error loading threats</td></tr>
          ) : threats.length === 0 ? (
            <tr><td colSpan={5} className="py-6 text-center text-gray-400">No threats found.</td></tr>
          ) : (
            threats.map((t, i) => (
              <tr key={i} onClick={() => onRowClick(t)} className="hover:bg-gray-50 cursor-pointer">
                <td className="py-2 px-4 truncate">{t.type}</td>
                <td className="py-2 px-4 truncate">{new Date(t.time).toISOString().replace("T", " ").slice(0, 19)}</td>
                <td className="py-2 px-4">
                  <span className={`text-white text-xs font-semibold px-2 py-1 rounded-full ${
                    t.severity === "high" ? "bg-red-500" :
                    t.severity === "medium" ? "bg-orange-500" : "bg-green-500"
                  }`}>
                    {t.severity}
                  </span>
                </td>
